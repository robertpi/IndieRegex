        ...
        public override IEnumerable<string> SampleMatches(int k, int randomseed)
        {
            lock (this)
            {
                ...
                for (int i = 0; i < k; i++)
                {
                    ...
                    // Here one could also consider previous characters for example for \b, \B, and ^ anchors
                    // and initialize inputSoFar accordingly
                    states.InitializeFrom(this, _initialStates[GetCharKind<FullInputReader>(ReadOnlySpan<char>.Empty, -1, FullInputReaderInstance)]);
                    CurrentState statesWrapper = new(states);
                    ...
                    while (true)
                    {
                        ...
                        // Gather the possible endings for satisfying nullability
                        possibleEndings.Clear();
                        if (SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.CanBeNullable(this, in statesWrapper))
                        {
                            // Unconditionally final state or end of the input due to \Z anchor for example
                            if (SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.IsNullable(this, in statesWrapper) ||
                                SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.IsNullableFor(this, in statesWrapper, CharKind.BeginningEnd))
                            {
                                possibleEndings.Add("");
                            }

                            // End of line due to end-of-line anchor
                            if (SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.IsNullableFor(this, in statesWrapper, CharKind.Newline))
                            {
                                possibleEndings.Add("\n");
                            }

                            // Related to wordborder due to \b or \B
                            if (SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.IsNullableFor(this, in statesWrapper, CharKind.WordLetter))
                            {
                                possibleEndings.Add(ChooseChar(random, asciiWordCharacters, ascii, charSetSolver).ToString());
                            }

                            // Related to wordborder due to \b or \B
                            if (SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.IsNullableFor(this, in statesWrapper, CharKind.General))
                            {
                                possibleEndings.Add(ChooseChar(random, asciiNonWordCharacters, ascii, charSetSolver).ToString());
                            }
                        }
                        ...
                        // Shuffle the minterms, including the last end-of-line marker if appropriate
                        int[] mintermIds = SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.StartsWithLineAnchor(this, in statesWrapper) ?
                            Shuffle(random, mintermIdsWithZ) :
                            Shuffle(random, mintermIdsWithoutZ);
                        foreach (int mintermId in mintermIds)
                        {
                            bool success = SymbolicRegexMatcher<TSet>.NfaStateHandlerInstance.TryTakeTransition(this, ref statesWrapper, mintermId);
                            Debug.Assert(success);
                            if (states.NfaStateSet.Count > 0)
                            {
                                TSet minterm = GetMintermFromId(mintermId);
                            ...
                            {
                                // The transition was a dead end, undo and continue to try another minterm
                                NfaStateHandlerInstance.UndoTransition(ref statesWrapper);
                            }
                        }
